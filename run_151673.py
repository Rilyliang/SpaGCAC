...
for dataset in data_list:
    ...
    if 'label_CSL' not in adata.obsm.keys():
        add_contrastive_label(adata)  # 形成N*2的数组，第一列全是1，第二列全是0
    ...
    if datatype in ['Stereo', 'Slide']:
        ...
        # standard version
        adj = preprocess_adj(adj)  # 标准化后加自环
    ...
    for epoch in tqdm(range(epochs)):
        ...
        # self.hiden_feat, self.emb, ret, ret_a, ret_mask = self.model(self.features, self.features_a, self.features_mask, self.adj) #Slide

        loss_sl_1 = loss_CSL(ret, label_CSL)
        loss_sl_2 = loss_CSL(ret_a, label_CSL)  # 破坏的损失
        loss_sl_3 = loss_CSL(ret_mask, label_CSL1)  # 掩码的损失
        loss_c = loss_cluster(c, c_mask)  # Slide  #聚类对比的损失
    ...
    savepath = 'D:/scientific study/paper1/SpaGCAC/' + dataset + '/'
    adata.write(savepath + 'SpaGCAC_sample_data.h5ad')

    with open(savepath + "ari_and_nmi.txt", "w") as file:
        file.write("ARI: {}\n".format(ARI))
        file.write("NMI: {}\n".format(NMI))

    title = 'SpaGCAC: ARI={:.3f}, NMI={:.3f}'.format(adata.uns['ARI'], adata.uns['NMI'])
    sc.pl.spatial(adata, img_key="hires", color=['domain'], title=title, show=False)
    plt.savefig(savepath + 'SpaGCAC.tif', bbox_inches='tight', dpi=300)
    ...
    sc.pl.paga_compare(adata, legend_fontsize=10, frameon=False, size=20, title=title, legend_fontoutline=2,
                       show=False)
    plt.savefig(savepath + 'SpaGCAC_PAGA_domain.tif', bbox_inches='tight', dpi=300)

    sc.tl.paga(adata, groups='ground_truth')
    sc.pl.paga_compare(adata, legend_fontsize=10, frameon=False, size=20, title=title, legend_fontoutline=2,
                       show=False)
    plt.savefig(savepath + 'SpaGCAC_PAGA_ground_truth.tif', bbox_inches='tight', dpi=300)
